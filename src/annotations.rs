...
pub struct AnnotationContainer {
    ...
    pub annotation_map: HashMap<String, HashSet<TermId>>, // Maps genes to directly annotated GO terms
    study_annotations: HashMap<TermId, usize>, // Maps GO terms annotated in the study set to their counts in the study set (nt)
    term_genes_map: HashMap<TermId, HashSet<String>>, // Maps GO terms annotated in the population set to their associated genes in the population set
}
...
impl AnnotationContainer {
    ...
    pub fn build_study_annotations(&mut self, study_set: &GeneSet, go: &FullCsrOntology) {
        ...
        for gene in study_set.gene_symbols() {
    ...
    pub fn build_term_genes_map(&mut self, pop_set: &GeneSet, go: &FullCsrOntology) {
        let mut term_to_genes: HashMap<TermId, HashSet<String>> = HashMap::new();
        let start = Instant::now();

        for gene in pop_set.gene_symbols() {
    ...
    pub fn annotation_map(&self) -> &HashMap<String, HashSet<TermId>> {
        &self.annotation_map
    }

    pub fn term_genes_map(&self) -> &HashMap<TermId, HashSet<String>> {
        &self.term_genes_map
    }
